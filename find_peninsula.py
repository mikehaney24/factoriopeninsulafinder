...
#
# The screen: cast rays outward from spawn. If a landmass is an island, it is
# bounded, so *every* ray must cross water to leave it. A single ray that
# reaches the window edge without hitting water is therefore proof the seed is
# not an island. This is exact for true islands; it is only a heuristic for
# partial peninsulas, which is why the prescan refuses to run at low
# --min-ratio (measured: seeds at ratio >= 0.50 had up to 12 clear rays, so a
# safe cutoff there retains 56% of seeds and buys nothing).
PRESCAN_RAYS = 64
PRESCAN_STEP = 1
PRESCAN_MIN_RATIO = 0.95
...
local RAYS, STEP, REACH = %(rays)d, %(step)d, %(reach)d

local function ray_points()
  local pts = {}
  for r = 1, RAYS do
    local a = (r - 1) * 2 * math.pi / RAYS
    for d = STEP, REACH, STEP do
...
script.on_init(function()
  local pts, per = ray_points(), REACH / STEP
...
def write_prescan_worker(worker_dir, start, count, reach, outfile):
...
def run_prescan(seed_start, seed_end, args, temp_base_dir, procs_lock, active_procs, shutdown_event):
    ...
    try:
        for i in range(workers):
            ...
            write_prescan_worker(wdir, start, count, reach, "rays.csv")
...
def main():
    ...
    parser.add_argument("--island-prescan", action="store_true",
                        help="Screen seeds with Factorio's noise directly (no preview rendering) and only render "
                             f"island candidates. ~30-130x faster, but sound only for islands, so it requires "
                             f"--min-ratio >= {PRESCAN_MIN_RATIO}.")
    ...
    parser.add_argument("--prescan-max-clear", type=int, default=0,
                        help="Keep seeds with at most this many rays reaching the window edge without crossing water "
                             "(default: 0, exact for true islands). Raise it to admit near-islands at higher cost.")
    ...
    if args.island_prescan and args.min_ratio < PRESCAN_MIN_RATIO:
        parser.error(
            f"--island-prescan requires --min-ratio >= {PRESCAN_MIN_RATIO} (got {args.min_ratio}).\n"
            "The prescan proves a seed is NOT an island by finding a straight path from spawn to the\n"
            "window edge that never crosses water. That is exact for islands, but a partial peninsula\n"
            "still connects to the edge across a wide arc, so the screen cannot separate one from open\n"
            "terrain: measured on 498 seeds, those scoring >= 0.50 had up to 12 clear rays, and a cutoff\n"
            "loose enough to keep them retains 56% of all seeds. Run without --island-prescan instead."
        )
    ...
    if args.island_prescan:
        print(f"{get_timestamp()} Island prescan: screening {total_to_scan} seeds from {args.start_seed} with "
    ...
    with tempfile.TemporaryDirectory(prefix="factorio_workers_", dir=args.temp_dir) as temp_base_dir:
        ...
        outer_workers = 1 if args.island_prescan else args.workers
    ...
    if args.island_prescan and stats["screened"]:
